...
import whoamiTxt from '../contents/whoami.txt';
...
export function getResult (showCommand) {
   switch (showCommand[0]) {
      ...
      case 'show ls':
         ...
         return final;
      case 'whoami':
         const whoami = fetch(whoamiTxt).then(res => res.text()).then(data => `${data}`.split(/\r?\n/));
         return whoami;
      case 'whoami error':
      ...
      case 'cat boot.log':
         const bootLog = fetch(bootLogTxt).then(res => res.text()).then(data => `${data}`.split(/\r?\n/));
         return bootLog;
      case 'cat directory':
